...
Each 6-minute video is divided into 6 1-minute clips.
"""
...
def process_video_file(csv_path, video_id):
    """
    Process a single video CSV file and divide it into 6 clips.
    ...
    # Divide into 6 equal clips
    frames_per_clip = total_frames / 6
    
    # Add video_id column
    df['video_id'] = video_id
    
    # Add clip_id column (1-6)
    clip_ids = (df.index / frames_per_clip).astype(int) + 1
    df['clip_id'] = np.minimum(clip_ids, 6)
    
    # Add a combined identifier
    df['sample_id'] = df['video_id'] + '_clip' + df['clip_id'].astype(str)
    
    print(f"Processed {video_id}: {total_frames} frames -> 6 clips")
...
def main():
    ...
    output_folder = Path("/Users/dyst/py-feat/features")
    ...
    for csv_file in csv_files:
        ...
        df = process_video_file(csv_file, video_id)
        all_data.append(df)
        print()
    ...
    print(f"  Total clips: {len(video_ids) * 6}")
    ...
    with open(summary_file, 'w') as f:
        f.write("AU Feature Extraction Summary\n")
        f.write("="*70 + "\n\n")
        f.write(f"Total videos: {len(video_ids)}\n")
        f.write(f"Total clips: {len(video_ids) * 6}\n")
        ...
        f.write("\n\n")
        f.write("Frames per clip:\n")
        f.write(clip_summary.to_string(index=False))
